...
We could choose to put a multinomial prior on $x$, or approximate $x$ with a continuous relative abundance
$$
x_i = p_iN
$$
where $p$ has a Dirichlet prior.  Substituting $pN$ into the model for $x_{reads}$ we get
$$
x_{reads} \sim \text{multinom}\left(N_{reads}, \frac{p\nu}{\sum_i p_i \nu_i}\right)
$$
...
modCode <- nimbleCode({
    ...
    # priors
    p[1:S] ~ ddirch(a0[1:S]) # a0 is defined in constants
    sigCopy ~ dunif(0, 100)
...
plot(x, sum(x)*colMeans(samp[, grep('p\\[', colnames(samp))]), bg = ordID, pch = 21, 
