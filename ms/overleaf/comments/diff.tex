...
%DIF DEL main_old.tex       Sun May  7 23:56:21 2017
%DIF ADD main_allEdit.tex   Sun May  7 23:54:15 2017
...
assembly. By presuming that populations \DIFdelbegin \DIFdel{do not differ in fitness nor
in }\DIFdelend \DIFaddbegin \DIFadd{are equivalent in regards to competition or }\DIFaddend resource use,
%DIF >  * <iovercast@gc.cuny.edu> 2017-05-07T19:07:55.628Z:
%DIF >  
%DIF >  > populations
%DIF >  Here I would say "individuals of different species within a trophic level" rather than 'populations'
%DIF >  
%DIF >  ^ <mhickerson@gmail.com> 2017-05-07T19:22:33.110Z:
%DIF > 
%DIF >  yes fitness is usually in the context of individuals within populations. Andy you might be conveying that populations are competitively equivalent   ("fitness" in the allele genotype analogy to genetic neutral theory)
%DIF > 
%DIF >  ^ <mhickerson@gmail.com> 2017-05-07T19:24:15.990Z.
...
\DIFdelbegin \DIFdel{Whatever mechanisms are at play, if they reach an equilibrium, statistical }\DIFdelend \DIFaddbegin \DIFadd{Statistical }\DIFaddend theories derive predictions by condensing the many bits of \DIFaddbegin \DIFadd{mechanistic }\DIFaddend information down into ecological state
variables \citep{Harte2011-um} and then mathematically maximizing the entropy of predicted distributions \DIFdelbegin \DIFdel{conditionally }\DIFdelend \DIFaddbegin \DIFadd{conditional }\DIFaddend on those state variables. \DIFdelbegin \DIFdel{One example is the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend maximum entropy theory of ecology (METE) \DIFdelbegin \DIFdel{which assumes that the area of an ecosystem, the total number of
...
capture all necessary information to characterize a community \mbox{%DIFAUXCMD
\citep{Harte2011-um}}%DIFAUXCMD
. While this
theory finds widespread success in predicting ahistorical patternsof
species abundance, body size, and spatial distribution }\DIFdelend \DIFaddbegin \DIFadd{is one such example \mbox{%DIFAUXCMD
\citep{Harte2011-um}}%DIFAUXCMD
. METE can predict many ahistorical patterns, not just one \mbox{%DIFAUXCMD
\citep{Harte2011-um}}%DIFAUXCMD
, and while taxonomically and geographically widespread testing supports the theory }\DIFaddend \citep{Harte2011-um,
...
commonly the species abundance distribution \citep{Hubbell2001-dx,
  Harte2011-um, White2012-yw}, although other metrics \DIFaddbegin \DIFadd{concerning body size, trophic links, and spatial distributions }\DIFaddend have been used
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Xiao2015-jv}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Xiao2015-jv, Rominger2015-kb, Harte2011-um}}%DIFAUXCMD
}\DIFaddend . To test \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{these theoretical predictions, }\DIFaddend we need a robust measure of
goodness of fit. The emerging consensus is that
likelihood-based test statistics should be preferred
\citep{baldridge2016}. The ``exact test'' of \DIFaddbegin \DIFadd{Etienne }\DIFaddend \citep{etienne2007} has
been extended by \DIFaddbegin \DIFadd{Rominger and Merow }\DIFaddend \citep{meteR} into a simple z-score which can
...
with equilibrial theory. While the fossil record \DIFdelbegin \DIFdel{can elucidate deep
time patterns for select, it is limited to  groups \mbox{%DIFAUXCMD
\citep{Alroy2008-no}}%DIFAUXCMD
, locales, and times \mbox{%DIFAUXCMD
\citep{Harnik2011-qe}}%DIFAUXCMD
, and therefore we require an approach that is applicable across taxa, space, and time. Population}\DIFdelend \DIFaddbegin \DIFadd{could be used for this task, it has limited temporal, spatial, and taxonomic resolution. Confronting these challenges in merging macroecology with paleontology is exciting and underway \mbox{%DIFAUXCMD
\citep{}}%DIFAUXCMD
. Here we instead focus on population}\DIFaddend /\DIFdelbegin \DIFdel{phylogenetics provide insights on }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic insights into }\DIFaddend rates of change of populations and species \DIFdelbegin \DIFdel{by processes of }\DIFdelend \DIFaddbegin \DIFadd{because of the detailed characterization of }\DIFaddend demographic fluctuations, immigration, selection, \DIFdelbegin \DIFdel{speciation, and extinction}\DIFdelend \DIFaddbegin \DIFadd{and speciation they provide}\DIFaddend . Bridging ecological theory with models from population/phylogenetics has great potential \DIFdelbegin \DIFdel{to generate general-use tools to gain insight into the dynamics
underlying contemporary biodiversity patterns }\DIFdelend \citep{Webb2002-yr, Emerson2002-mw, Lavergne2010-ts, Li2016-ns, McGaughran2015-sy, Laroche2015-qo, Vanoverbeke2015-ym, Vellend2005-qd, Papadopoulou2011-bd,Dexter2012-rn} \DIFdelbegin \DIFdel{. However, bridging of these approaches }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend has yet to \DIFdelbegin \DIFdel{quantitatively }\DIFdelend \DIFaddbegin \DIFadd{be fully realized. How we can best }\DIFaddend link the inferences of change through time from
%DIF >  * <mhickerson@gmail.com> 2017-05-07T19:48:23.164Z:
%DIF >  
%DIF >  missing citations?
%DIF >  
%DIF >  ^.
phylo/population genetics with inferences \DIFdelbegin \DIFdel{of macroscopic ahistorical
patterns from macroecology .  If we can overcome this shortfall, it would be possible to
finally test which biodiversity patterns we see today results from equilibrium or non-equilibrium}\DIFdelend \DIFaddbegin \DIFadd{from macroecology depends on what specific insights we can glean from genetic perspectives on demography and diversification}\DIFaddend .

One of the fundamental \DIFdelbegin \DIFdel{tool }\DIFdelend \DIFaddbegin \DIFadd{tools }\DIFaddend allowing for complex historical inference
with population genetic data is coalescent theory
\citep{Hudson1983-hx, Tajima1983-me, Kingman1982-uf, Kingman1982-ie,
  Rosenberg2002-ag}.  \DIFdelbegin \DIFdel{Now broadly applied, coalescent theory can
...
virtually any complex demographic history.  }\DIFdelend Coalescent theory allows for model-based
...
approach can also be put in a multi-species, community context \DIFdelbegin \DIFdel{thanks
to advances in historical }\DIFdelend \DIFaddbegin \DIFadd{via hierarchical }\DIFaddend demographic models \citep{Xue2015-el,
...
These modeled demographic deviations from neutral demographic equilibrium \DIFdelbegin \DIFdel{can also }\DIFdelend \DIFaddbegin \DIFadd{could potentially }\DIFaddend be condensed to \DIFdelbegin \DIFdel{summary statistics, as in the case with ecological theory. }\DIFdelend \DIFaddbegin \DIFadd{multi-species summary statistics similar to how }\DIFaddend Tajima's D \DIFaddbegin \DIFadd{detects selection or demographic expansion in a single species. In this case it }\DIFaddend measures the standardized difference between two theoretical derivations of genetic diversity that should be equal under neutral equilibrium, but different when non-equilibrium demography dominates \citep{Tajima1989-mc, Freedman2016-yx, Barton1998-xs, Barton2000-gr, Jensen2005-ef, Schrider2016-cw, Stephan2016-lf, Good2014-fq}.
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  * <mhickerson@gmail.com> 2017-05-07T21:44:24.800Z:
%DIF >  
%DIF >  without looking at the box, these two options seem kind of similar
%DIF >  
%DIF >  ^.
...
\DIFdelbegin \DIFdel{population demographics}\DIFdelend \DIFaddbegin \DIFadd{community-level population genetics}\DIFaddend . Assuming these two approaches are within
...
%DIF >  * <mhickerson@gmail.com> 2017-05-07T21:49:08.313Z:
%DIF >  
%DIF >  it will be interesting to see if we get away with citing Tajima's D here like this as there is so much to umpack with this statement. 
%DIF >  
%DIF >  ^.
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  * <mhickerson@gmail.com> 2017-05-07T21:50:56.394Z:
%DIF >  
%DIF >  oh yeah, there is a typo in the Figure 1 x axis of the 4 panels. It should be 'Demographic OR phylogenetic departure from equilibrium'
%DIF >  
%DIF >  ^.
...
%DIFDELCMD < %%%
\DIFdelend Next generation sequencing \DIFaddbegin \DIFadd{(NGS) }\DIFaddend technology has ushered in a revolution in evolutionary biology and ecology.  \DIFaddbegin \DIFadd{Whole genome and transcriptome sequences can now be generated even for non-model organisms (Ellegren 2014) and tools like RAD sequencing (Hohenlohe et al. 2015) or ultraconserved elements (Faircloth et al. 2012) allow researchers to study intra - and interspecific genetic variation of thousands of loci in parallel. This progress led to fascinating new insights into the structure of the tree of life (Misof et al. 2014) and the genetic basis of adaptation and speciation (Jones et al. 2012; Soria-Carrasco et al. 2014). 
%DIF >  * <krehenwinkel@berkeley.edu> 2017-05-08T02:45:55.769Z:
%DIF >  
%DIF >  Additional references for the section:
%DIF >  Misof, Bernhard, et al. "Phylogenomics resolves the timing and pattern of insect evolution." Science 346.6210 (2014): 763-767.
%DIF >  Faircloth, Brant C., et al. "Ultraconserved elements anchor thousands of genetic markers spanning multiple evolutionary timescales." Systematic biology (2012): sys004.
%DIF >  Hohenlohe, Paul A., et al. "Next‐generation RAD sequencing identifies thousands of SNPs for assessing hybridization between rainbow and westslope cutthroat trout." Molecular ecology resources 11.s1 (2011): 117-122.
%DIF >  Soria-Carrasco, Víctor, et al. "Stick insect genomes reveal natural selection’s role in parallel speciation." Science 344.6185 (2014): 738-742.
%DIF >  Ellegren, Hans. "Genome sequencing and population genomics in non-model organisms." Trends in ecology & evolution 29.1 (2014): 51-63.
%DIF >  Jones, Felicity C., et al. "The genomic basis of adaptive evolution in threespine sticklebacks." Nature 484.7392 (2012): 55-61.
%DIF >  
%DIF >  
%DIF >  
%DIF >  
%DIF >  ^.
Current phylogenomic and population genomic studies are usually based on large numbers of loci for a limited number of taxa or populations. This tactic is reversed in NGS based community analyses, which allow researchers to characterize the composition of complex species communities based on a limited set of informative barcode markers. }\DIFaddend The large scale recovery from bulk
...
\DIFdelbegin \DIFdel{simulates an }\DIFdelend \DIFaddbegin \DIFadd{jointly models a forward-time }\DIFaddend individual-based \DIFdelbegin \DIFdel{forward time community dispersal model
\mbox{%DIFAUXCMD
\citep{Rosindell2015-dk} }%DIFAUXCMD
linked with }\DIFdelend \DIFaddbegin \DIFadd{neutral community assembly process
\mbox{%DIFAUXCMD
\citep{Rosindell2015-dk} }%DIFAUXCMD
and corresponding expectations of community level genetic 
diversity and divergence using }\DIFaddend the msPrime coalescent simulator
...
information about the historical processes that gave rise to them\DIFaddbegin \DIFadd{.
}\DIFaddend 

\paragraph{Approximate Bayesian computation (ABC)} \DIFdelbegin \DIFdel{. }\DIFdelend A method of
...
\paragraph{coalescent} A \DIFaddbegin \DIFadd{stochastic, }\DIFaddend backwards in time \DIFdelbegin \DIFdel{model approach in population genetics in }\DIFdelend \DIFaddbegin \DIFadd{population genetic model 
in }\DIFaddend which alleles in the sample are traced to their ancestors under demographic models of interest.
...
\item How can functional genomics be used to better distinguish
  ...
  non-equilibrium? Can understanding the \DIFdelbegin \DIFdel{functional }\DIFdelend \DIFaddbegin \DIFadd{gene }\DIFaddend content of genomes\DIFaddbegin \DIFadd{, gene expression patterns or occurrence of mutations
}\DIFaddend across taxa in a community help predict potential for
  non-equilibrium responses to future perturbations? 
\DIFdelbegin \DIFdel{Functional
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  * <iovercast@gc.cuny.edu> 2017-05-07T22:23:17.276Z:
%DIF >  
%DIF >  Is there some way to clarify in the figure that departure from equilibrium is minimized toward the bottom left and maximized toward the top right? In the original figure i feel like there as an arrow along each axis indicating the direction of increasing disequilibrium. 
%DIF >  
%DIF >  ^.
\begin{figure}[!hbp]
  \centering
